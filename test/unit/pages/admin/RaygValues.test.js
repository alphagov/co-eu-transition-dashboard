...
describe('pages/admin/rayg-values/RaygValues', () => {
  ...
  describe('#buildEntitiesToBeSaved', () => {
    it('should return correctly formated entity data', async () => {
      const body = { '123': 'red', '234': 'green' }
      const entities = { '123': { publicId: 123,  categoryId :2 }, '234': { publicId: 234,  categoryId :2 } }
      const response = await page.buildEntitiesToBeSaved(body, entities);
      expect(response).to.eql([{ publicId: 123, categoryId: 2, raygStatus: 'red' }, { publicId: 234, categoryId: 2, raygStatus: 'green' }]);
    });
  });

  describe('#flatternEntityData', () => {
    it('should return a flattern entity data', async () => {
      const entities = [{ id: 123,   categoryId :2, children: [ { id: 234, categoryId :2 }, { id: 345, categoryId :3 } ] }]
      const response = await page.flatternEntityData(entities);
      expect(response).to.eql({ 123: { id: 123, categoryId: 2 }, 234: { id: 234, categoryId: 2 }, 345: { id: 345, categoryId :3 } } );
    });
  });

  describe('#postRequest', () => {
    beforeEach(() => {
      sinon.stub(page, 'getThemesAndTopLevelStatements').returns([{ id: 123, publicId: 'theme-1', categoryId :2, children: [ { id: 234, publicId: 'state-1', categoryId :2 }, { id: 345, publicId: 'state-2', categoryId :3 } ] }]);
    ...
    it('should call flash and redirect on error ', async () => {
      req = { body: { 678: 'red' }, originalUrl: 'someurl', flash: sinon.stub()  }; 
    ...
    it('should call saveData when no errors returned', async () => {
      req = { body: { 123: 'red' }, originalUrl: 'someurl', flash: sinon.stub()  }; 
       await page.postRequest(req, res);
       sinon.assert.calledWith(page.saveData, [{ publicId: 'theme-1', categoryId: 2, raygStatus: 'red' }]);
    });
  });
  ...
  describe('#saveData', () => {
    ...
    const dataToSave = [{ publicId: "B", categoryId: 1, raygStatus: "red", id: 123 }]
    ...
    it('should call entity import and redirect', async () => {
      page.req = { body: { 678: 'red' }, originalUrl: 'someurl', flash: sinon.stub()  }; 
